...
如需让命令名以 `;` 或 `#` 开头，或在字段内保留字面量 `:`，请使用反斜杠转义，例如：

```
\;:"C:\Tool\Notepad3.exe"
name:C\:\Tool\demo.exe
```

参数说明：
- **命令名**：显示在启动器中的名称
- **命令行**：要执行的完整命令（支持带引号的路径，支持 `%SystemRoot%`、`%SCOOP%` 等环境变量）
  - 可执行文件路径本身不含空格时，可直接写成 `C:\Tool\Notepad3.exe /utf8`
  - 可执行文件路径含空格时，路径部分仍需加引号，例如 `"C:\Program Files\Notepad++\notepad++.exe" /multiInst`
- **运行路径**：工作目录（可为空，支持环境变量）
